...
The native bullet exposes both `getDamage()` and `getArmorPenetration()`; the shipped Model 700 reports `damage=65` and `armorPenetration=11`. RC27 uses those interfaces only to collapse the bounded one/two/three-point drone structure more quickly. It does not import actor-health scaling or create a parallel hit-point system.
