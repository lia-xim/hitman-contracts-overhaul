...
The game owns body discovery through `worldObject:getInteractionQuadTree()` before it reaches `goon.interactionList`, `entity:getInteractOptions`, `entity:updateInteractionList`, `currentActionBitmask`, `entity:postInteract`, `getInteractPriority`, and the object selector's validate/render sequence. RC47 restores an eligible fallen actor to the current native interaction tree when a direct death/checkpoint path omitted it. RC46's sentinel IDs, per-body ordering, temporary priority and active-player render reconciliation remain unchanged.
...
5. Before refreshing actions, ensure an eligible body is registered in the current world's interaction quadtree; never reinsert an already healthy entry.
6. Let the native object selector own its single `postInteract` call after takeover or restoration.
7. Never expose takeover without an active HCO contract, a player interactor, an eligible dead/unconscious Goon, a valid appearance variant, and an unused source identity. Expose restoration only while an identity is active.
