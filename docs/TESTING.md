...
These checks do not prove live AI behavior or rendering. A release candidate is promoted only after this in-game pass on Intravenous 2 1.4.12HF3. RC29 intentionally disables the old RC render diagnostic in normal play; an internal `HCO RC.. DRONE ROSTER` HUD message is now a failure, not an expected result.
