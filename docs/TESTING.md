...
RC21 specifically reports `HCO RC21 FLIGHT MODEL`. The expected values are `quadtree ACTIVE`, `batch READY`, `sprite READY`, and at least one body. The four fields isolate world-entity visibility, native batch creation, asset loading, and spawn state respectively.
...
6. Verify that passive patrol drones have a roughly actor-sized animated four-rotor body above the active searchlight. The sensor nose must face with the cone. Hover bob, rotor pulses and a short cyan pixel wake should make movement readable, and rotor audio should react to distance. Three suppressed player shots and NPC fire must not trigger aggressive mode.
...
10. Shoot down a normal drone. Its body, light and loop sound must stop, a local crash alert must occur and available response guards should investigate the crash position. Commander Hunter armor may require two hits.
11. Resolve the target and verify the compact completion banner, sound, payout, and persistence after reload.
