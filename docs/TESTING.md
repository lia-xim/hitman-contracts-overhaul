...
These checks do not prove live AI behavior or rendering. A release candidate is promoted only after this in-game pass on Intravenous 2 1.4.12HF3. Release builds intentionally disable the internal render diagnostic in normal play; an `HCO RC.. DRONE ROSTER` HUD message is a failure, not an expected result.
...
8. With no disguise or exposed cover, stand visibly inside a drone cone for at least one second. Verify the cone changes red, stays attached to the player instead of sweeping away, `DRONE CONTACT` appears, and response units receive the last-known position. With a clean newly acquired disguise, repeat beyond 205 world units during an alarm: the cone must remain amber and cannot track, confirm or fire. Move inside 155 units during patrol or 205 during an alarm and verify progressive close scrutiny before red confirmation. Move around a confirmed drone and verify gimbal-first/body-second rotation plus a stable standoff slot.
...
15. Leave a dead/unconscious NPC in an unobstructed patrol cone. Verify one `DRONE EVIDENCE SCAN` cue and an expanding scan pulse. Repeated passes must not duplicate the report. If it is the source of the active uniform, that disguise must become compromised; disruption must suppress the scan. Then observe a wall-following drone for at least 30 seconds: it must keep one avoidance side, change routes when local movement makes no destination progress and never remain in an unreachable firing pocket.
...
17. During routine, observe the protected target and all five close guards for at least 60 seconds. On a populated compatible map, the target must traverse at least five visibly distinct locations across several sectors rather than shuttle between A and B; each close guard must move in the same protection chain. Reload the checkpoint and verify the same seeded route returns. A nine-second stationary condition must advance the route without leaving the actor pathless.
18. Let an armed drone confirm the current appearance, break LOS for more than ten seconds, then re-enter a clear firing lane. It must reacquire and resume fire. Kill nearby HCO response guards and/or the principal and repeat; an intact airframe must retain its native weapon attribution and continue attacking.
19. Restart the mission, create a fresh dead or unconscious eligible Goon and open the native body selector. `Take disguise / search body` must again be the first available identity action; stale options or action bits from the prior run must not hide it.
