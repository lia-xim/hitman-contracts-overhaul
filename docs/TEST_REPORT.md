# Automated Test Report — 0.13.2-rc31
...
- Lua syntax: **PASS** — all 32 Lua modules parse, including the RC31 terminal-destruction gate, RC30 roof/fairness gate and RC29 balance snapshot.
...
- Post-test evidence collector: **PASS** — installed `0.13.2-rc31`, 32 source Lua files, 32 installed Lua files and 0 mismatches. The game was not running; the newest stored crash log predates this installation and the current debug log is empty.
...
- Workshop ZIP integrity: **PASS** — `Hitman-Contracts-Overhaul-0.13.2-rc31.zip` contains all 42 payload files below the required nested `files/` root and has SHA-256 `1C31863102CFE199FD9B750AFBC047D69716B045D3D5AFBF51005AF945FDF5C6`.
- Repository release gate: **PASS** — `HCO_RELEASE_CHECK_PASS version=0.13.2-rc31 payload=42`.

The markers above were collected from RC31 source and its exact local installation. Automated results prove internal behavior, packaging and failure handling only; they do not replace the final live-mission pass.
