...
**Implementation checkpoint (`0.14.6-rc40`, 2026-08-07):** Phases 0–5 and the seven-model physical-drone portion of Phase 6 are present in the RC source and covered by syntax, simulated runtime, lifecycle, persistence, failure-isolation and rollback tests. RC24–RC33 established compact, outdoor, shootable, armed and persistently wrecked drone airframes. RC34 rebuilds Phase 3/4 social stealth on the real Goon interaction ID/cache path. RC35–RC37 corrected takeover, weapon-neutrality and native sight bypasses. RC38 hardens close-protection follower ownership and separates alert presentation from identity knowledge. RC39 adds FOV/raycast-gated close scrutiny, point-blank exposure and a consequential native hostile handoff so social stealth creates routes without trivializing them. RC40 commits patrol destinations until arrival, exhausts outdoor fallbacks, retains a 360-degree sensor scan when boxed in and shares confirmed drone contact across all HCO security contexts while retaining per-drone LOS fire authority. The exact specification-to-code/test/live matrix is `SPECIFICATION_TRACEABILITY.md`. A crash-free live acceptance pass remains mandatory. Thermal cameras, dedicated operators, authored private-area allowlists and expanded contract families remain incomplete expansions. Automated results are not substitutes for real-game proof.
