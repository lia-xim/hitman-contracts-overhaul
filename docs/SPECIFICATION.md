...
**Implementation checkpoint (`0.14.10-rc44`, 2026-08-07):** Phases 0–5 and the seven-model physical-drone portion of Phase 6 are present in the RC source and covered by syntax, simulated runtime, lifecycle, persistence, failure-isolation and rollback tests. RC24–RC33 established compact, outdoor, shootable, armed and persistently wrecked drone airframes. RC34 rebuilds Phase 3/4 social stealth on the real Goon interaction ID/cache path. RC35–RC37 corrected takeover, weapon-neutrality and native sight bypasses. RC38 hardens close-protection follower ownership and separates alert presentation from identity knowledge. RC39 adds FOV/raycast-gated close scrutiny, point-blank exposure and a consequential native hostile handoff. RC40 commits patrol destinations, exhausts outdoor fallbacks, retains a 360-degree boxed-in scan and shares confirmed contact across HCO security contexts. RC41 adds bounded disarmed exterior-barrier flight, principal patrol/incident watchdogs, first-position disguise takeover, explicit original-identity restoration and a persistent player identity shimmer. RC42 prevents aggressive location searches from becoming automatic identity knowledge, scopes confirmation to the observed appearance, adds amber long-range/close progressive drone scrutiny and detects wall motion that makes no destination progress. RC43 preserves the path produced by the native target patrol callback, converts close protection into one verified follower chain, sustains armed-drone reacquisition/attribution after stale reports and casualties, and repairs missing disguise actions across mission/class/menu lifecycle resets. RC44 reconciles takeover and restore directly at the native Goon `getInteractOptions` query so an `update=false` cached selector cannot hide a valid action behind an already-set bit. The exact specification-to-code/test/live matrix is `SPECIFICATION_TRACEABILITY.md`. A crash-free live acceptance pass remains mandatory. Thermal cameras, dedicated operators, authored private-area allowlists and expanded contract families remain incomplete expansions. Automated results are not substitutes for real-game proof.
...
The protected principal must remain a mobile systemic actor. A routine target that makes no progress for nine seconds is advanced through its original native patrol route. The first nearby unsuppressed shot creates cautious location-level awareness without identifying the shooter. Confirmed protection damage, casualties or communicated contact escalate the existing retreat/evacuation phases, and a fresh incident near occupied shelter forces reselection.
