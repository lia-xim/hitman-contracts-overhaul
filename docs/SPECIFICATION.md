...
The native Goon sight states are allowed to calculate and present suspicion, but they are not allowed to convert a clean disguise into player-specific hostility through a close-range shortcut. While the active identity is neither locally nor globally compromised and the player is not performing an overt action, every instantiated `onSightHitPlayer` path must retain native detection progression without executing direct combat, spotted-stat or `setEnemyInSight(true, player)` branches. A second class-level enemy-sight boundary must fail closed if an unpatched or future state attempts the same transition.

Ordinary social suspicion is capped immediately below the explicit same-unit identity-check threshold. The timed check may cross that threshold and open a real interruptible radio report; close range by itself may not. A witnessed aim, shot, lock break, takedown or other overt action passes through to native combat. A directly observed clothes change establishes local knowledge of the new identity. An unwitnessed change instead clears the prior actor-specific detection meter, sight/hearing target, enemy-sight maps and persistent `seenPlayer` memory so the old identity cannot leak into the new one. Restoring an already active clean disguise from a checkpoint performs the same cleanup without fabricating a clothes-change witness.
