# Completion Audit — 0.13.2-rc32

**Audit date:** 2026-08-07
**Goal status:** production-candidate code complete for the current feature surface; final live acceptance is not complete
**Authority inspected:** RC32 source, `SPECIFICATION.md`, current smoke harnesses, decompiled `1.4.12HF3` follower/camera/projectile/damage/outline/physics/path-grid/roof interfaces and user-supplied live screenshots/errors
...
| Drone EMP/destruction | RC32 retains terminal cleanup and adds durable native wreck-batch plus pre-insertion projectile-sweep coverage | Latest live pass proved destroyed drones became inert but visually vanished; some Heavy free-fire edge shots required right-click selection | RC32 live pass required |
...
5. Live-approve RC32 native roof-map placement, outdoor footprint recovery, unaided complete-silhouette hits and unmistakable inert wreck handoff; meaningful operator/radio dependency remains a future expansion.
...
RC32 is approved as a production candidate for local/Workshop testing, not yet as final `1.0`. Automated proof now retains the wreck through a separate registered native batch and covers the complete first projectile sweep from the recorded muzzle position. The next pass must prove that a light and Heavy wreck remain visible after landing and that free shots across Heavy rotor edges register without right-click selection, then continue the broader matrix without traceback. The exact promotion matrix is maintained in `PRODUCTION_READINESS.md`.
