...
RC47 is approved for local production-candidate testing, not yet as final `1.0`. The next live report clarified that the marked body could not be cycled to at all: the selector discovered only its dropped weapon/equipment. RC47 now repairs the missing native interaction-quadtree registration before RC46's priority/cache logic runs. The next pass must prove a fresh body becomes object 1 and immediately exposes takeover after a complete restart.
