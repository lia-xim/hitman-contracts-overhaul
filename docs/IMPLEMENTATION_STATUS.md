...
| Disguise and social stealth | RC45 candidate path: weapon-neutral distance cover, close scrutiny, point-blank exposure, first-visible native takeover, exact validate/render selector semantics, stable native action IDs, explicit rollback, body-availability stitches, persistent identity shimmer and mission-lifecycle repair | Full real-mission pass in `SPECIFICATION_TRACEABILITY.md` |
...
## RC22 roster / RC23–RC45 live corrections
...
- RC44 closes the remaining visible-menu gap at the authoritative Goon `getInteractOptions` call. Even an `update=false` cached result with an already-set action bit is reconciled before the native selector consumes it, while non-HCO options and ordering remain native-owned.
...
1. Completely restart the game and confirm `0.14.11-rc45` loads without traceback and without an internal RC render diagnostic on the HUD.
