...
local body={x=90,y=110}
function body:getPos() return self.x,self.y end
function body:getDrawPosition() return self.x,self.y end
local bodyLinesBefore,bodyPixelsBefore=identityLines,identityPixels
assert(identityFX.drawAvailableBody(body,false),"eligible body identity marker renders")
assert(identityLines>=bodyLinesBefore+8 and identityPixels==bodyPixelsBefore+3,"eligible body marker uses restrained world-space stitches")
identityFX.trigger(state,"acquired")
identityFX.update(state,0.2)
player:postDraw()
assert(identityLines>18 and identityPixels==bodyPixelsBefore+11,"identity transition renders segmented native-world geometry")
...
player:postDraw()
assert(identityPixels==bodyPixelsBefore+27,"compromise transition renders a final pixel burst")
